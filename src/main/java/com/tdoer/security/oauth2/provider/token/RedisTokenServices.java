...
/**
 * Copy and modify {@link org.springframework.security.oauth2.provider.token.DefaultTokenServices},
 * The token service will use {@link RedisTokenStore}
 * for extension features. -- Likai Hu, 2018/10/13.
 *
...
public class RedisTokenServices implements AuthorizationServerTokenServices, ResourceServerTokenServices,
        ConsumerTokenServices, ReadingRefreshTokenServices, InitializingBean {

	private int refreshTokenValiditySeconds = 60 * 60 * 24 * 30; // default 30 days.

	private int accessTokenValiditySeconds = 60 * 60 * 12; // default 12 hours.

    // ResourceServerTokenService has no need to set it
	...
	// AuthorizationServerTokenService should set it
    // ResourceServerTokenService has no need to set it
    ...
    // AuthorizationServerTokenService, which support refresh token, must set it
    // ResourceServerTokenService has no need to set it
	...
	public OAuth2AccessToken createAccessToken(OAuth2Authentication authentication) throws AuthenticationException {
        ...
        SessionPolicy sessionPolicy = env.getClientConfig().getTokenConfig().getSessionPolicy();
        ...
        switch (sessionPolicy){
            case KICK_OFF_PREVIOUS:
                // create a new token and mark previous existing tokens from the approval
                // that are kicked off by the new token
                OAuth2AccessToken newToken = createToken(authentication);
                Collection<OAuth2AccessToken> approvedTokens = tokenStore.findTokensByClientIdAndUserName(clientId, userName);
                if(approvedTokens != null && approvedTokens.size() > 0){
                    for(OAuth2AccessToken token : approvedTokens){

                        // exclude the newly-created token
                        if(!newToken.getValue().equals(token.getValue())){
                            token.getAdditionalInformation().put(OAuth2Constants.KICKED_OFF_BY, newToken.getValue());
                            token.getAdditionalInformation().put(OAuth2Constants.KICKED_OFF_ON, new Date());
                            // save back to store
                            tokenStore.saveBackAccessToken(token);
                        }
                    }
                }
                return newToken;
            case BLOCK_NEWLY_COMING:
                // failed to create a new token, blocked by the existing token
                approvedTokens = tokenStore.findTokensByClientIdAndUserName(clientId, userName);
                if(approvedTokens == null || approvedTokens.size() ==0){
                    ...
                    for(OAuth2AccessToken token : approvedTokens){
                        ...
                        if(auth != null){
                            OAuth2AccessBlockedException abe = new OAuth2AccessBlockedException("Failed to create a new token for there has been existing authentication");
                            OAuth2AuthenticationUtil.collectBlockInfoFromAuthenctication(auth, abe);
                            throw abe;
                        }else {
                            // no authentication
                    ...
                    // if no authentication found, create a new token
                    return createToken(authentication);
                }
            default: // allow all
	...
	protected OAuth2AccessToken createToken(OAuth2Authentication authentication){
        OAuth2RefreshToken refreshToken = null;
        OAuth2AccessToken existingAccessToken = tokenStore.getAccessToken(authentication);
        if (existingAccessToken != null) {
            if (existingAccessToken.isExpired() || existingAccessToken.getAdditionalInformation().get(OAuth2Constants.KICKED_OFF_BY) != null) {
                // Existing access token is expired or replaced, remove all about it in store
	...
	protected  void removeAccessToken(OAuth2AccessToken accessToken){
	...
	public OAuth2RefreshToken readRefreshTokenForToken(String tokenValue) {
		return tokenStore.readRefreshTokenForToken(tokenValue);
	}
	...
	private OAuth2AccessToken createAccessToken(OAuth2Authentication authentication, OAuth2RefreshToken refreshToken) {
		...
		token.getAdditionalInformation().put(OAuth2Constants.CREATED_ON, new Date());
